...
class Managelayout extends CI_Controller
{
     ...
     * 상점 레이아웃관리합니다
     */
    function shop($config = array(), $device_view_type = '')
    {
        $data = array();
        $CI = & get_instance();
        $tmp = dir(APPPATH . 'config');
        $menu_files = array();
        while ($entry = $tmp->read()) {
            if (!preg_match('/^shop_menu_([0-9]{3}).*\.php$/', $entry, $m))
                continue;
        ...
        $data['shop_page_menu'] = array();
        foreach($menu_files as $file){
            $CI->load->config($file);
            $res = config_item('shop_page_menu');
            $data['shop_page_menu'] += config_item('shop_page_menu');
        }


        if (uri_string() !== config_item('uri_segment_shop')) {
            list($data['menu_dir0'], $data['menu_dir1'], $data['menu_dir2']) = explode('/', uri_string());
        } else {
            $data['menu_dir0'] = '';
            $data['menu_dir1'] = '';
         ...
         * 메뉴 이름 가져오기 - 2020.02.17 ARK
         */
        foreach ($data['shop_page_menu'] as $key=>$val){
            if($data['menu_dir1'] == $key){
                $data['menu_dir1_name'] = element(0, element('__config', $val));
                foreach (element('menu', $val) as $s_key=>$s_val){
                    if($data['menu_dir2'] == $s_key){
        ...
        $layout = 'shop/_layout/' . $layoutdirname;
        $data['layout_skin_path'] = $layout;
        $data['layout_skin_url'] = base_url( VIEW_DIR . $data['layout_skin_path']);
